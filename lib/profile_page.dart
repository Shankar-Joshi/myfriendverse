...
class ProfilePage extends StatefulWidget {
  const ProfilePage({this.userId, this.auth, this.mUserpro});

  final String userId;
  final Auth auth;
  final User mUserpro;

  _ProfilePage createState() => _ProfilePage(this.userId, this.auth, this.mUserpro);
}
...
class _ProfilePage extends State<ProfilePage>
  ...
  final User proUser;
  _ProfilePage(this.profileId, this._auth, this.proUser);
  ...
  Future<User>firestoreUser() async{
    ...
    DocumentSnapshot dsnap = await ref.document(profileId).get();

  setState(() {
    this.myusermodel = User.fromDocument(dsnap);
  });

  }

  editProfile() {
    EditProfilePage editPage = EditProfilePage(signedcurrentUserModel);
  ...
  followUser() {
    ...
    Firestore.instance
        ...
        .setData({
      "ownerId": profileId,
      "username": signedcurrentUserModel.username,
      "userId": currentUserId,
      "type": "follow",
      "userProfileImg": signedcurrentUserModel.photoUrl,
      "timestamp": DateTime.now().toString()
    });

    Firestore.instance
        .collection("follow_details")
        .document(currentUserId)
        .collection('friends')
        .document(profileId)
        .setData({
      ...
      "bio": myusermodel.bio,
      "onlinetime": myusermodel.onlinetime,
      "photoUrl": myusermodel.photoUrl,
      "following": profileId,
      "type": "follow",
      "timestamp": DateTime.now().toString()
    });
    Firestore.instance
      ...
      "ownerId": profileId,
      "username": signedcurrentUserModel.username,
      "displayName": signedcurrentUserModel.displayName,
      "email": signedcurrentUserModel.email,
      "bio": signedcurrentUserModel.bio,
      "onlinetime": signedcurrentUserModel.onlinetime,
      "photoUrl": signedcurrentUserModel.photoUrl,
